...
int createVertexBufferObject2() {
    ...
    int slices = 20;
    float x, y, z;
    std::vector<vec3> vertexArray2;
    
    for(int i = 0; i <= stacks; i) {
        float phi = PI * i / stacks; // Polar angle (phi) from 0 to PI
        float sinPhi = std::sin(phi);
        float cosPhi = std::cos(phi);
         for (int j = 0; j <= slices; j++) {
            float theta = 2 * PI * j / slices; // Azimuthal angle (theta) from 0 to 2*PI
